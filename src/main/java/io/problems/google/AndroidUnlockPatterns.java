...
public class AndroidUnlockPatterns {
  public int numberOfPatterns(int m, int n) {
    ...
    // initialize indices of skip matrix (all other indices in matrix are 0
    // by default)
    skip[1][3] = skip[3][1] = 2;
    skip[1][7] = skip[7][1] = 4;
    skip[3][9] = skip[9][3] = 6;
    ...
    for (int i = m; i <= n; ++i) {
      totalNumber += DFS(visited, skip, 1, i - 1) * 4; // 1, 3, 7, and 9
                                                       // are symmetric so
                                                       // multiply this
                                                       // result by 4
      totalNumber += DFS(visited, skip, 2, i - 1) * 4; // 2, 4, 6, and 8
                                                       // are symmetric so
                                                       // multiply this
                                                       // result by 4
      totalNumber += DFS(visited, skip, 5, i - 1); // do not multiply by 4
                                                   // because 5 is unique
    }
  ...
  int DFS(boolean visited[], int[][] skip, int current, int remaining) {
    ...
    for (int i = 1; i <= 9; ++i) {
      // if the current node has not been visited and (two numbers are
      // adjacent or skip number has already been visited)
